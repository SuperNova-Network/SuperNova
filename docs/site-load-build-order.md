...
- Static assets are served from the built `dist/` directory (generated by Vite) with cache headers. The default `pnpm start` runs the Vite build first.
...
- Frontend is built with Vite (`pnpm build`, automatically run by `pnpm start`), which outputs hashed assets to `dist/`.
- Deployment copies the repository (including `dist/`, or by rebuilding) and runs `pnpm start` to serve the built assets.
